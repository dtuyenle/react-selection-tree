...
const chevrondown = 'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIzMiIgaGVpZ2h0PSIzMiIgdmlld0JveD0iMCAwIDMyIDMyIj48cGF0aCBkPSJNMTYuMDAzIDE4LjYyNmw3LjA4MS03LjA4MUwyNSAxMy40NmwtOC45OTcgOC45OTgtOS4wMDMtOSAxLjkxNy0xLjkxNnoiLz48L3N2Zz4=';
...
const CheckboxLabelLevelDown = styled.img`
  width: auto;
  height: 15px;
`;
...
export default class InputBox extends Component {
    ...
    if (this.state.search_value.trim().length < 3) {
      keys.forEach((key) => {
        if (!filter.children[key].stayfirst) {
          DATA.push(<div key={key}>{this.getCheckBox(filter.children[key], name)}</div>);
        } else {
          DATASTAYFIRST.push(<div key={key}>{this.getCheckBox(filter.children[key], name)}</div>);
        }
      });
    } else {
      if (filter.search_result && filter.search_result.length > 0) {
        filter.search_result.forEach((item, index) => {
          if (!item.stayfirst) {
            DATA.push(<div key={index}>{this.getCheckBox(item, name)}</div>);
          } else {
            DATASTAYFIRST.push(<div key={index}>{this.getCheckBox(item, name)}</div>);
          }
        });
      }
    }
  ...
  getCheckBox(checkboxNode, filterName) {
    ...
    const label = (<CheckboxLabel htmlFor={id} title={hoverText}>
      ...
      >
        <CheckboxLabelLevelDown alt="next-level" src={chevrondown}></CheckboxLabelLevelDown>
      </span> }
    </CheckboxLabel>);
