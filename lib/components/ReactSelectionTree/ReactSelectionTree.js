  ...
  _createClass(ReactSelectionTree, [{
    ...
    value: function update(data, callback) {
      ...
      this.setState({
        ...
        if (callback) {
          callback(data, _this2.state.flatData);
        }
      });
    }
  }, {
      ...
      return _react2.default.createElement(
        ...
        !this.checkEmpty() && this.state.data && _react2.default.createElement(_InputBox2.default, {
          ...
          checkFilter: function checkFilter(name, filter, checked) {
            ...
            _this3.update(filter.parent && filter.parent !== null ? filter.parent : filter, _this3.props.check);
          },
          updateFilter: function updateFilter(data) {
            _this3.update(data);
            _this3.props.breadcrumClick(data);
          },
          searchFilter: function searchFilter(value) {
            var filter = _this3.state.data;
            var result = [];
            Object.keys(_this3.state.flatData).forEach(function (key) {
              if (result.length < 15 && _this3.state.flatData[key].name.toLowerCase().indexOf(value.toLowerCase()) !== -1) {
                result.push(_this3.state.flatData[key]);
              }
            });
            while (filter.parent && filter.parent !== null) {
              filter = _this3.state.data.parent;
            }
            filter.search_result = result;
            _this3.update(filter, _this3.props.search);
          },
          resetSearch: function resetSearch() {
            ...
            _this3.update(filter, _this3.props.reset);
          },
          uncheckAll: function uncheckAll() {
            var filter = _this3.state.data;
            var flatData = _this3.state.flatData;
            Object.keys(flatData).forEach(function (key) {
              flatData[key].checked = false;
            });
            while (filter.parent && filter.parent !== null) {
            ...
            _this3.update(filter, _this3.props.uncheckAll);
          },
